...
public class BoxMaze extends UnicastRemoteObject implements BoxMazeInterface
{
    ...
    private Vector<ClientInterface> clientQue = new Vector<ClientInterface>();
    private final Object obj1 = new Object();
    private final Object obj2 = new Object();
    private Vector<PositionInMaze> clientPos;
	...
	public void registerClientCallBack(ClientInterface clientCallBackObject)throws RemoteException {
		...
		synchronized (obj2) {
			if (!(clientQue.contains(clientCallBackObject))) {
				clientQue.addElement(clientCallBackObject);
			} // end if
			System.out.println("New Registration on RMIServer:" + clientQue.size());
		}
	}

	@Override
	public synchronized void unRegisterClientCallBack(ClientInterface clientCallRemoveBackObject) throws RemoteException {
		// TODO Auto-generated method stub
		//System.out.println("Someone lefted them maze");
		clientQue.removeElement(clientCallRemoveBackObject);
		clientQue.lastElement().sendMsg("Client Disconnected from server");
	}
	
	public void SendCallbacks() throws RemoteException {
		synchronized(obj2) {
			
			boolean[][] temp = new boolean[Maze.DIM][Maze.DIM];
			
			for(PositionInMaze m : clientPos){
				temp[m.getXpos()][m.getYpos()] = true;
			}
			
			for(ClientInterface c : clientQue) {
				c.setClientPossitions(temp);
